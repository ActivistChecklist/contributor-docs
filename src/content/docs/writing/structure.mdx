...
![Page layout](../../../assets/structure-page-layout.png)
...
4. **Sections organized by risk level**: (See [below for more details](#sections).)
8. **Cross-links:** End each guide with "Keep learning with these related guides" linking to 2-3 relevant checklists.
1. **Recommended guides:** List 2-4 guides that are related to the current guide.

## Sections 

**Section ordering**: If a guide uses risk levels, order sections as everyone, then medium threat, then high threat. (Note: You can have more than one section for a risk level. Example: the [Essentials](https://activistchecklist.org/essentials/) guide has a "get started" section before the main "everyone" section.)
...
![Checklist item layout](../../../assets/structure-checklist-layout.png)
...
### `<HowTo>` boxes
Use this format in `<HowTo>` boxes for actionable instructions.
